...
def execute_httpSend(sReq ,sReq2 ):
	cHttp = http_func.http_funcClass()
	cHttp.send_push(sReq ,1)
	#2
	cHttp.send_push(sReq2  ,2)
...
def send_http(clsDat):
	...
	if (len(sV1 ) > 0 ):
		iV1 = int(sV1)
		sReq +="&value1=" + str(iV1)
		sReq2+="&field1=" + str(iV1)
	sV2=clsDat.get_datByName("device2" , 1)
	if (len(sV2 ) > 0 ):
		iV2 = int(sV2)
		sReq +="&value2=" + str(iV2)
		sReq2+="&field2=" + str(iV2)
	sV2b=clsDat.get_datByName("device2" , 2 )
	if (len(sV2b ) > 0 ):
		iV2b = int(sV2b )
		sReq +="&value3=" + str(iV2b )
		sReq2+="&field3=" + str(iV2b )
	sV3=clsDat.get_datByName("device3" , 1 )
	if (len(sV3 ) > 0 ):
		iV3 = int(sV3)
		sReq +="&value4=" + str(iV3 )
		sReq2+="&field4=" + str(iV3)
	sV4=clsDat.get_datByName("device4" , 1 )
	if (len(sV4 ) > 0 ):
		iV4 = int(sV4)
		sReq +="&value5=" + str(iV4 )
		sReq2+="&field5=" + str(iV4)
	
	th=threading.Thread(target=execute_httpSend ,args=(sReq, sReq2 ) )
	th.start()
